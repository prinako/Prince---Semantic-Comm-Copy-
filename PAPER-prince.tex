...
We consider a text semantic communication system in which a pretrained semantic transmitter is retained while the receiver is adapted to recover semantic information in a target language. The communication architecture follows the DeepSC paradigm and consists of a semantic encoder, a channel encoder, a physical wireless channel, a channel decoder, and a Transformer-based semantic decoder.
...
Feature-level information is also transferred between the Transformer decoder representations. Let $\mathbf{f}_T$ and $\mathbf{f}_S$ denote the Teacher and Student decoder features at a valid token position. Their alignment is encouraged using a cosine-based feature loss,
\begin{equation}
    \mathcal{L}_{\mathrm{feat}}
    =
    1-
    \frac{
        \mathbf{f}_S^{\mathrm{T}}\mathbf{f}_T
    }{
        \lVert\mathbf{f}_S\rVert_2
        \lVert\mathbf{f}_T\rVert_2
    }.
\end{equation}
Padding positions are excluded when the feature loss is aggregated across a sequence.
...
After knowledge distillation, the trained Student receiver is used as the base model for language-specific adaptation. Instead of fully fine-tuning the receiver, Low-Rank Adaptation (LoRA) is introduced into selected attention projections of the Student Transformer decoder.
...
In the proposed receiver, LoRA is applied to the query, key, value, and output projections of both the decoder self-attention and encoder--decoder attention modules. The base attention weights remain frozen, while only the low-rank matrices are optimized. The implementation additionally permits the output prediction head and LayerNorm parameters to be adapted while the embedding layer remains frozen.
...
Knowledge distillation is not repeated during this stage. Instead, the Student first inherits the Teacher knowledge through the distillation stage and is subsequently specialized to Portuguese through parameter-efficient LoRA adaptation. The complete training pipeline can therefore be summarized as
\begin{equation}
    \text{Pretrained DeepSC}
    \rightarrow
    \text{KD Student Receiver}
    \rightarrow
    \text{LoRA Language Adapter}.
\end{equation}
